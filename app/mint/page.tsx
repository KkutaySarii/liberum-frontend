...
const MintPage = () => {
  ...
  return (
    <div className='w-full h-screen bg-dark overflow-auto noscrollbar'>
      <NavbarMint/>

      <main className="container max-w-5xl mx-auto mt-12 noscrollbar">
        <div className='pt-32 text-center'> 
          <h1 className="text-5xl font-bold mb-4">Mint Your Blockspace</h1>
          <p className={`text-white font-satoshi-light font-thin text-stroke-thin text-base mb-12 leading-relaxed tracking-wider transition-all duration-300 ${
          ...
          <div className={`relative max-w-xl mx-auto mb-16 transition-all duration-500 ${
            isInputFocused ? '-translate-y-20' : ''
          }`}>
            <input 
              type="text"
              placeholder="search for a name"
              className="w-full px-6 py-3 rounded-lg bg-light border border-light-white text-black placeholder:text-black placeholder:!font-thin placeholder:text-2xl placeholder:text-opacity-40 outline-none"
              onFocus={() => setIsInputFocused(true)}
            />
            <Image src={SearchIcon} alt="Search" width={30} height={30} className='absolute right-4 top-1/2 -translate-y-1/2' />
          </div>

          <div className={`grid grid-cols-3 gap-10 relative transition-all duration-500 ${
          ...
          <div className={`absolute top-[45%] pb-20 right-1/2 w-2/5 translate-x-1/2 -translate-y-1/2 transition-all duration-500 ${
            isInputFocused ? 'block translate-y-0 opacity-100' : 'hidden translate-y-full opacity-0'
          }`}>
